    ...
    if (state == projectState::GAME && project) {
      ...
      if (const auto *wheelEvent =
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::LSystem) ||
          ...
          //   sf::Vector2u size =
          //   project->getToolBar().getSelectedTool()->getSize(); float sizex =
          //   size.x; float sizey = size.y; sf::Vector2u mapSize =
          //   project->getMap()->getSize(); float step =
          //   std::min(mapSize.x,mapSize.y)/100.0f;
          //
          ...
          //
          //   size =  sf::Vector2u(static_cast<unsigned
          //   int>(std::round(sizex)), static_cast<unsigned
          //   int>(std::round(sizey)));
