...
long long DatabaseManager::verifyLogin(const std::string& pseudo, const std::string& password) {
...
bool DatabaseManager::addLink(const long long userId, const long long projectId, const int8_t role) {
    QSqlQuery query;
    query.prepare("INSERT INTO links (user_id, project_id, role) VALUES (:u, :p, :r)");
...
long long DatabaseManager::addProject(const std::string& name, const long long userId) {
...
bool DatabaseManager::changeRole(const long long userId, const long long projectId, const int8_t role) {
    ...
    query.prepare("UPDATE links SET role = :r WHERE user_id = :u AND project_id = :p");
    query.bindValue(":r", static_cast<int>(role));
    query.bindValue(":u", static_cast<qlonglong>(userId));
    query.bindValue(":p", static_cast<qlonglong>(projectId));
...
std::vector<MemberEntry> DatabaseManager::getProjectMembers(long long projectId) {
...
int8_t DatabaseManager::getRole(const long long userId, const long long projectId) {
    QSqlQuery query;

    query.prepare("SELECT role FROM links WHERE user_id = :u AND project_id = :p");
    query.bindValue(":u", userId);
    query.bindValue(":p", projectId);
