...
class Zoom {
    ...
    public :
        void setFacteurZoom(const float zoom) { facteurZoom_ = zoom; }
        float getZoom() const { return facteurZoom_; }
        void zoomIn() { // zoom sur la carte
            facteurZoom_ *= 0.9f;
            if (facteurZoom_ < minZoom_) { // check le fait de ne pas dépasser la borne inférieur
                facteurZoom_ = minZoom_;
            }
        }
        void zoomOut() { // dézoom sur la carte
            facteurZoom_ *= 1.1f;
