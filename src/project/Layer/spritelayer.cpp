...
SpriteLayer::SpriteLayer(std::string name, sf::Vector2u size)
    : Layer(name, size) {
  type_ = SPRITELAYER;
}
...
SpriteObject::SpriteObject(const sf::Sprite& sprite, uint64_t id): sprite{sprite},id{id}{}
...
void SpriteLayer::draw(const sf::Sprite &s) {
  sprites_.push_back(SpriteObject(s,nextId));
  nextId ++;}

void SpriteLayer::erase(uint64_t id) {
